...
def _conv_layer(x, num_filters, filter_size, strides, relu=True):
    ...
    x = tf.nn.conv2d(x, weights_init, strides_shape, padding='SAME')
    x = _instance_norm(x) # 加上BN
    if relu:
        x = tf.nn.relu(x)
    return x

def _instance_norm(net, training =True):
...
def _conv_init_vars(net, out_channels, filter_size, transpose=False):
    ...
    weights_init = tf.Variable(tf.truncated_normal(weights_shape, stddev=1, seed=1), dtype=tf.float32)
    return weights_init
